...
router.post('/components/freebox', function (req, res, next) {
  let upload = multer({ storage: storage }).single('images/freebox')
  upload(req, res, (err) => {
    if (err) {
      console.error(err)
      let date = new Date()
      ...
      res.status(503).send('Could not upload new freebox to main page.')
    } else {
      let title = req.body.title
      let text = req.body.text
      let link = req.body.link
      ...
      fs.readFile('static/common/content.json', 'utf8', (err, data) => {
        if (err) {
          ...
          let result = JSON.parse(data)
          let id = result.mainPage.freeboxes[0].id
          for (var i = 0; i < result.mainPage.freeboxes.length; i++) {
            if (result.mainPage.freeboxes[i].id > id) {
              id = result.mainPage.freeboxes[i].id
            }
          }
          let newFreebox = {
            id: id + 1,
            freeboxImg: req.file ? req.file.originalname : '',
            freeboxTitle: title,
            freeboxText: text,
            freeboxLink: link,
            freeboxBg: bg,
            order: result.mainPage.freeboxes.length + 1
          }
          result.mainPage.freeboxes.push(newFreebox)
          fs.writeFile('static/common/content.json', JSON.stringify(result), 'utf-8', (err, data) => {
            if (err) {
              console.log(err)
              let date = new Date()
              ...
              res.status(503).send('Could not write to file content.json main page new freebox.')
            } else {
              res.status(200).send({'status': 200})
            }
          })
